...
function ModalShell({
  ...
  return createPortal(
    ...
    >
      <section
        className="flex max-h-[calc(100%-2rem)] w-full max-w-[380px] flex-col rounded-lg border border-border bg-card text-card-foreground shadow-2xl"
        role="dialog"
        aria-modal="true"
        aria-label={title}
        data-testid={testId}
...
export function RenameDialog({
  ...
  return (
    <ModalShell title={t('recentFiles.renameTitle')} onClose={onClose} testId="rename-dialog">
      <input
        ref={inputRef}
        className="h-8 w-full rounded-md border border-input bg-background px-2.5 text-[13px] outline-none focus:ring-2 focus:ring-ring"
        value={value}
...
export function ShareDialog({ file, onClose }: { file: RecentFile; onClose: () => void }) {
  ...
  const optionClass =
    'flex w-full items-center gap-3 rounded-md border border-border px-3 py-2.5 text-left hover:bg-accent'
