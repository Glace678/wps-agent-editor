...
export function FileSearch({ query, onQueryChange, results, onOpen, isSearching }: FileSearchProps) {
  ...
  return (
    <div className="flex min-w-0 flex-col">
      ...
      */}
      <div className="min-w-0 w-full px-1.5 pb-2">
        <div
          className={cn(
            'flex h-9 min-w-0 w-full items-center gap-0 rounded-xl border border-input bg-transparent shadow-xs',
            ...
            <button
              ...
              onDragStart={(event) => {
                writeAgentAttachmentDragData(
                  event.dataTransfer,
                  [createAgentAttachment(file.path, 'browse')],
                )
              }}
            >
