...
export function RecentFileContextMenu({ x, y, onAction, onClose }: RecentFileContextMenuProps) {
  ...
  const [coords, setCoords] = useState<MenuCoords>({ top: y, left: x, anchorX: x, anchorY: y })
  const [isDark, setIsDark] = useState(readDocumentIsDark)

  // 首帧直接显示在指针处；下一帧再测量并翻转，避免同步布局测量阻塞右键反馈。
  useEffect(() => {
    setCoords((current) => (
      current.anchorX === x && current.anchorY === y
        ? current
        : { top: y, left: x, anchorX: x, anchorY: y }
    ))
    const frame = requestAnimationFrame(() => {
      ...
      setCoords({ top, left, anchorX: x, anchorY: y })
    })
  ...
  return createPortal(
    <div
      ...
      style={{
        position: 'fixed',
        top: coords.anchorX === x && coords.anchorY === y ? coords.top : y,
        left: coords.anchorX === x && coords.anchorY === y ? coords.left : x,
        visibility: 'visible',
        zIndex: 2147483000,
        minWidth: 184,
        borderRadius: 12,
    ...
    >
      {MENU_GROUPS.map((group, groupIndex) => (
        <div key={group[0].action} role="group" className="px-1.5">
          ...
          {group.map(({ action, labelKey, icon: Icon, danger }) => (
            <button
              ...
              className={cn(
                'flex w-full items-center gap-2.5 rounded-md px-2.5 py-1.5 text-left text-[13px] hover:bg-accent focus:bg-accent focus:outline-none',
