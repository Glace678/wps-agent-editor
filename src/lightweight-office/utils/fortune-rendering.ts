...
function getSharedSpreadsheetFontFamilies(
  ...
  // Shared menu order (Chinese first by pinyin, then Western A→Z), except that
  // the spreadsheet default stays pinned at index zero: Fortune Sheet renders
  // cells without an explicit font in fontarray[0].
  const entries = getOrderedFontFamilyEntries(fontFaces, {
    pinFirst: DEFAULT_OFFICE_FONT_FAMILY,
  })
  const usedMenuNames = new Set<string>()

  return entries.map(({ familyName, displayName }) => {
    const menuDisplayName = displayName.trim() || familyName
    const displayKey = normalizeSystemFontFamilyName(menuDisplayName)
