...
import { buildFontSearchTerms, normalizeFontSearchText } from '../utils/font-search'
...
export function ExcelEditor({ filePath, onReady, onDirty, onSaveSuccess, onRegisterSave }: ExcelEditorProps) {
  ...
  useEffect(() => {
    ...
    return attachExcelToolbarPopupBoundary(shell)
  }, [sheets, fontLibraryReady])
  ...
  useEffect(() => {
    ...
    // Shared with the Word picker: family/display names + pinyin aliases.
    const fontSearchTerms = buildFontSearchTerms(fontFaces)
    ...
    const decoratePicker = (popup: HTMLElement) => {
      ...
      if (kind === 'font-size') {
        // Font size dropdown: no search bar; display choices directly (Requirement 1)
        popup.dataset.excelPickerSearchReady = 'true'
        popup.dataset.excelPickerKind = 'font-size'

        popup.style.whiteSpace = 'normal'
        popup.style.minWidth = '52px'
        popup.style.width = '56px'
        popup.style.maxWidth = '64px'
        popup.style.boxSizing = 'border-box'
        popup.style.overflow = 'hidden'
        select.style.minWidth = '100%'
        select.style.width = '100%'
        ...
        // Check if 8 is in options; if not, prepend 8 (Image 1)
        const existingOptions = Array.from(select.querySelectorAll<HTMLElement>('.fortune-toolbar-select-option'))
        const has8 = existingOptions.some((opt) => opt.textContent?.trim() === '8')
        if (!has8) {
          const opt8 = document.createElement('div')
          opt8.className = 'fortune-toolbar-select-option'
          opt8.tabIndex = 0
          const textDiv = document.createElement('div')
          textDiv.className = 'fortuen-toolbar-text'
          textDiv.textContent = '8'
          opt8.appendChild(textDiv)
          select.prepend(opt8)
        }
        ...
        const applyFontSize = (size: number) => {
          ...
          if (api && selection?.length) {
            api.setCellFormatByRange('fs', size, selection)
            scheduleExcelCanvasRefresh()
          }
          const textSpan = container.querySelector<HTMLElement>('.fortune-toolbar-combo-text')
          if (textSpan) textSpan.textContent = String(size)
          closePicker()
        }

        const options = Array.from(select.querySelectorAll<HTMLElement>('.fortune-toolbar-select-option'))
        for (const option of options) {
          const valStr = option.textContent?.trim() || ''
          const size = parseExcelFontSizeForPicker(valStr)
          const isActive = valStr === comboText || (size !== null && Number(comboText) === size)
          option.classList.toggle('excel-toolbar-picker-option-active', isActive)
        ...
        const activeOption = options.find((opt) => opt.classList.contains('excel-toolbar-picker-option-active'))
        if (activeOption) {
          requestAnimationFrame(() => {
      ...
      input.setAttribute(
        'data-testid',
        kind === 'font' ? 'excel-font-search' : 'excel-format-search',
      )
    ...
    const isFontSizeComboContainer = (container: HTMLElement | null): boolean => {
      ...
      const label = [button?.getAttribute('aria-label'), button?.dataset.tips]
        ...
        .join(' ')
        .toLocaleLowerCase()
      if (EXCEL_FONT_SIZE_LABEL_RE.test(label)) return true
      const text = container.querySelector<HTMLElement>('.fortune-toolbar-combo-text')?.textContent?.trim()
      if (text && /^\d+(?:\.\d+)?$/.test(text)) return true
      return false
    }

    const startFontSizeInlineEdit = (container: HTMLElement, button: HTMLElement) => {
      const combo = button.closest<HTMLElement>('.fortune-toolbar-combo')
      const textSpan = button.querySelector<HTMLElement>('.fortune-toolbar-combo-text')
      const currentText = textSpan?.textContent?.trim() || '11'

      // Check if already editing
      let input = button.querySelector<HTMLInputElement>('.excel-font-size-toolbar-input')
      ...
      container.dataset.excelFontSizeEditing = 'true'
      button.dataset.excelFontSizeEditing = 'true'
      if (combo) combo.dataset.excelFontSizeEditing = 'true'
      ...
      if (!input) {
        ...
        input.type = 'text'
        input.className = 'excel-font-size-toolbar-input'
        input.autocomplete = 'off'
        input.spellcheck = false
        input.setAttribute('aria-label', 'Font Size')
        input.setAttribute('data-testid', 'excel-font-size-toolbar-input')
      ...
      const api = workbookRef.current
      const selectionAtStart = api?.getSelection()
      ...
      const commit = () => {
        ...
        const size = parseExcelFontSizeForPicker(val)
        if (size !== null && api) {
          const targetSel = (selectionAtStart && selectionAtStart.length > 0)
            ? selectionAtStart
            : api.getSelection()
          if (targetSel?.length) {
            api.setCellFormatByRange('fs', size, targetSel)
            if (textSpan) textSpan.textContent = String(size)
            scheduleExcelCanvasRefresh()
          }
        }
      ...
      const cleanup = () => {
        ...
        delete container.dataset.excelFontSizeEditing
        delete button.dataset.excelFontSizeEditing
        if (combo) delete combo.dataset.excelFontSizeEditing
      }
      ...
      const onBlur = (e: FocusEvent) => {
        if (e.relatedTarget && button.contains(e.relatedTarget as Node)) return
        commit()
      }
    ...
    const handleFontSizeButtonPointerDown = (event: MouseEvent) => {
      const target = event.target as HTMLElement | null
      if (!target) return
      if (target.classList.contains('excel-font-size-toolbar-input')) return

      const button = target.closest<HTMLElement>('.fortune-toolbar-combo-button')
      if (!button) return
      const container = button.closest<HTMLElement>('.fortune-toobar-combo-container')
      if (!container || !isFontSizeComboContainer(container)) return

      event.preventDefault()
      event.stopPropagation()

      const openPopup = container.querySelector<HTMLElement>('.fortune-toolbar-combo-popup')
      if (openPopup) {
        container.querySelector<HTMLElement>('.fortune-toolbar-combo-arrow')?.click()
      }

      startFontSizeInlineEdit(container, button)
    }

    const handleFontSizeButtonClick = (event: MouseEvent) => {
      const target = event.target as HTMLElement | null
      if (!target) return
      if (target.classList.contains('excel-font-size-toolbar-input')) return
    ...
    const handleColorButtonClick = (event: MouseEvent) => {
      const target = event.target as HTMLElement | null
      if (!target) return
      if (target.closest('.fortune-toolbar-combo-popup, #fortune-custom-color, #fortune-change-color')) return

      const button = target.closest<HTMLElement>('.fortune-toolbar-combo-button, .fortune-toolbar-combo')
      if (!button) return
      const container = button.closest<HTMLElement>('.fortune-toobar-combo-container')
      if (!container || !isExcelColorCombo(container)) return

      if (target.closest('.fortune-toolbar-combo-arrow')) return

      event.preventDefault()
      event.stopPropagation()

      const arrow = container.querySelector<HTMLElement>('.fortune-toolbar-combo-arrow')
      arrow?.click()
    }

    const decorateOpenPickers = () => {
      shell.querySelectorAll<HTMLElement>('.fortune-toobar-combo-container').forEach((container) => {
        if (isExcelFontColorCombo(container)) {
          container.dataset.excelColorCombo = 'font-color'
        } else if (isExcelBgColorCombo(container)) {
          container.dataset.excelColorCombo = 'background'
        }
      })
    ...
    shell.addEventListener('mousedown', handleFontSizeButtonPointerDown, true)
    shell.addEventListener('click', handleFontSizeButtonClick, true)
    shell.addEventListener('click', handleColorButtonClick, true)
    shell.addEventListener('click', handleToolbarComboClick, true)
    decorateOpenPickers()

    return () => {
      pickerObserver.disconnect()
      shell.removeEventListener('mousedown', handleFontSizeButtonPointerDown, true)
      shell.removeEventListener('click', handleFontSizeButtonClick, true)
      shell.removeEventListener('click', handleColorButtonClick, true)
