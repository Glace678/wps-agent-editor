...
import { getSystemFontFamilyNames } from './utils/system-fonts'
...
export function createFullWordEditorModules(fontFaces: SystemFontFace[]) {
  const fonts = getSystemFontFamilyNames(fontFaces).map((familyName) => ({
    // SuperDoc applies `label` to the document. Use the same canonical name
    // stored by Notepad and Excel instead of a localized presentation label.
    label: familyName,
    key: familyName,
