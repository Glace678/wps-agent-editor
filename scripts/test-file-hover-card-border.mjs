/**
 * Theme-aware outline on FileHoverCard (right-side popup for 浏览).
...
test('only the browse tree uses FileHoverCard for the popup', () => {
  assert.match(read('src/components/file-manager/FileTree.tsx'), /FileHoverCard/)
  assert.doesNotMatch(read('src/components/file-manager/RecentFiles.tsx'), /FileHoverCard/)
})
