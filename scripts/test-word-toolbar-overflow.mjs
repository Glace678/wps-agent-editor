...
test('the current font name is centered in the toolbar combobox', () => {
  assert.match(
    css,
    /\.superdoc-toolbar\s+\.sd-font-combobox__input\s*\{[^}]*text-align:\s*center/,
  )
})
...
test('the Word font-size combobox uses a compact width without changing the font-family combobox', () => {
  ...
  assert.match(css, /:has\(\.button-text-input--font-size\)[\s\S]*?\.sd-toolbar-split-field__caret\s*\{[\s\S]*?width:\s*16px\s*!important/)
