...
test('right-clicking any row selects it and opens the menu', () => {
  assert.match(source, /event\.preventDefault\(\)[\s\S]*if \(!selectedPaths\.has\(file\.path\)\) setSelectedPaths\(new Set\(\[file\.path\]\)\)[\s\S]*setMenu/)
