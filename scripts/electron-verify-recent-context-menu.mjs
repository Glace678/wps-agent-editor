...
try {
  ...
  await waitFor(cdp.send, `Boolean(document.querySelector('[data-testid=\"recent-file-context-menu\"]'))`, 'unselected row context menu open', 8000)
  const unselectedRowSelected = await evaluate(
    cdp.send,
    `Boolean(${recentRowExpr(path.basename(missingFile))}?.getAttribute('aria-selected') === 'true')`,
  )
  await cdp.send('Input.dispatchKeyEvent', { type: 'keyDown', key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27 })
  await cdp.send('Input.dispatchKeyEvent', { type: 'keyUp', key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27 })
  await waitFor(cdp.send, `!document.querySelector('[data-testid=\"recent-file-context-menu\"]')`, 'unselected row menu closed', 5000)
  ...
  await evaluate(
    cdp.send,
    `(() => {
      for (const checkbox of document.querySelectorAll('[data-recent-file-select]')) {
  ...
  record(
    'V1b checkboxes multi-select with the single-file menu',
      ...
      && unselectedRowSelected && themeAware && selectedCount === 3
      && JSON.stringify(multiMenuItems) === JSON.stringify(menuItems),
    `idle=${checkboxIdleBeforeHover?.opacity} hover=${checkboxVisibleOnHover} pointerEvents=${checkboxIdleBeforeHover?.pointerEvents} unselectedSelected=${unselectedRowSelected} theme=${themeAware} colors=${JSON.stringify(themeColors)} selected=${selectedCount}`,
  )
  // ---------- V2: file info dialog ----------
