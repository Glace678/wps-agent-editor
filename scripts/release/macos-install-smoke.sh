...
if [[ $# -lt 4 ]]; then
  echo 'Usage: macos-install-smoke.sh <dmg> <x86_64|aarch64> <log-directory> <updater-app.tar.gz>' >&2
  exit 2
fi
...
cat >"$log_dir/macos-$arch-install-smoke.json" <<EOF
{
  ...
  "arch": "$arch",
  "dmg": "$(basename "$dmg")",
  "updaterArchive": "$(basename "$updater_archive")",
  ...
  "updaterContentVerified": true,
  "updaterMatchesDmg": true,
...
mounted=0
echo "macOS mount/install/start/remove smoke passed for $arch"
