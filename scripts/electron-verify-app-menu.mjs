...
try {
  ...
  assert.equal(
    await evaluate(cdp, "Boolean(document.querySelector('[data-testid=app-menu-action-run-multi-agent]'))"),
    false,
    'multi-Agent collaboration must be absent from the Agent menu',
  )
  assert.equal(
    await evaluate(cdp, "Boolean(document.querySelector('[data-testid=collaboration-open]'))"),
    false,
    'multi-Agent collaboration must be absent from the Agent assistant',
  )
  const collaborationResult = await evaluate(
    cdp,
    "window.api.agent.runTask(['disabled-a', 'disabled-b'], 'must not run')",
  )
  assert.equal(
    typeof collaborationResult?.error,
    'string',
    'the main process must reject legacy multi-Agent requests while collaboration is paused',
  )
