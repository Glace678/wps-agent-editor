...
test('tier-0 overflow menu absorbs heading and list styles', () => {
  const start = src.indexOf('function OverflowFormattingMenu')
  const end = src.indexOf('function useFormattingTier')
  assert.ok(start !== -1 && end > start, 'OverflowFormattingMenu must exist')
  const overflow = src.slice(start, end)
  assert.match(overflow, /\{tier < 1 && \(/)
  for (const cmd of [
    'paragraph', 'heading-1', 'heading-2', 'heading-3',
    'bullet-list', 'number-list', 'check-list',
  ]) {
    assert.match(overflow, new RegExp(`onFormat\\('${cmd}'\\)`), `overflow must offer ${cmd}`)
  }
})
