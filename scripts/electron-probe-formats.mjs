...
const sampleDocx = process.env.WPS_PROBE_DOCX
const sampleDoc = process.env.WPS_PROBE_DOC
const sampleXlsx = path.join(os.tmpdir(), `wps-agent-editor-smoke-${process.pid}.xlsx`)
...
try {
  ...
  for (const file of [sampleDocx, sampleDoc, sampleXlsx].filter(Boolean)) {
